...
class Simulator(nn.Module):
    ...
    def _get_pre_target(self, inputs: Dataset) -> torch.Tensor:
        return inputs.x[:, self.output_index_start : self.output_index_end]

    def _build_input_graph(self, inputs: Dataset, is_training: bool) -> Dataset:
    ...
    def _build_outputs(
        self, inputs: Dataset, network_output: torch.Tensor
    ) -> torch.Tensor:
        pre_target = self._get_pre_target(inputs)
